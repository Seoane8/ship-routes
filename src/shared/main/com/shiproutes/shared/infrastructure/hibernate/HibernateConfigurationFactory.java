...
public final class HibernateConfigurationFactory {
    ...
    public DataSource dataSource(
        ...
        String mysqlSentences = new Scanner(mysqlResource.getInputStream(), StandardCharsets.UTF_8).useDelimiter("\\A"
        ).next();
    ...
    private List<String> subdirectoriesFor(String contextName) {
        String path = "./src/" + contextName + "/main/com/shiproutes/" + contextName + "/";

        String[] files = new File(path).list((current, name) -> new File(current, name).isDirectory());

        if (null == files) {
            path = "./main/com/shiproutes/" + contextName + "/";
